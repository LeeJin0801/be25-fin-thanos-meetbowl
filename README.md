...
|   윤정윤   | Meetbowl 프로젝트는 여러 서버와 기능이 연결된 구조에서 전체 흐름과 책임 분리를 고민해 본 경험이었습니다. 특히 실시간 STT는 빠른 화면 표시 중심으로, 회의록 생성은 최종 원문 기반의 비동기 처리로 분리하면서 기능별 요구사항에 맞는 설계가 중요하다는 것을 배웠습니다.
또한 API 서버와 AI 서버의 역할을 나누고, RabbitMQ를 활용해 작업을 안정적으로 처리하면서 서버 간 책임 경계를 명확히 하는 경험을 했습니다. 협업 측면에서는 하네스와 문서를 통해 API 응답, 에러 코드, 이벤트 구조, 배포 기준 등을 표준화하며 팀원이 같은 기준으로 개발할 수 있는 환경을 만들고자 했습니다.
이번 프로젝트를 통해 백엔드 개발자는 단순히 API를 구현하는 것을 넘어 시스템 흐름, 장애 대응, 배포 안정성, 협업 표준까지 함께 고려해야 한다는 것을 배웠습니다.|
